...
function SubtitlesContent() {
  ...
  const lastRecordedSubtitleKeyRef = useRef<string | null>(null)
  ...
  useEffect(() => {
    if (!wordLearningConfig.enabled || !subtitle?.text)
      return

    const subtitleKey = `${subtitle.start}:${subtitle.end}:${subtitle.text}`
    if (lastRecordedSubtitleKeyRef.current === subtitleKey)
      return

    lastRecordedSubtitleKeyRef.current = subtitleKey
    void recordLearningTextEncounter(subtitle.text, "subtitles", {
    ...
    subtitle?.start,
    subtitle?.text,
    wordLearningConfig.enabled,
    wordLearningConfig.hideKnownWords,
    wordLearningConfig.minimumLevel,
  ])
