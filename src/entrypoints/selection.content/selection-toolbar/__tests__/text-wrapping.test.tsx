...
describe("selection toolbar text wrapping", () => {
  ...
  it("renders escaped newlines as line breaks in selection translation text", () => {
    render(
      <TooltipProvider>
        <TranslationContent
          selectionContent="source"
          translatedText={"第一行\\n第二行"}
          isTranslating={false}
          minimumWordLearningLevel="cet4"
          thinking={null}
    ...
    const translatedNode = document.querySelectorAll("p")[1]
    expect(translatedNode).toHaveTextContent("第一行")
    expect(translatedNode?.textContent).toBe("第一行\n第二行")
  })
  ...
  it("renders escaped newlines as line breaks in structured object fields", () => {
    render(
      <StructuredObjectRenderer
        outputSchema={[
          {
        ...
        value={{ Definition: "第一行\\n第二行" }}
        thinking={null}
      />,
    )

    const fieldValue = document.querySelector("[data-field-name='Definition'] [class*='whitespace-pre-wrap']")
    expect(fieldValue).toHaveTextContent("第一行")
    expect(fieldValue?.textContent).toBe("第一行\n第二行")
  })
