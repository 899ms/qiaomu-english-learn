...
function createVisibleCustomActionConfig() {
  ...
  const action = config.selectionToolbar.customActions[0]
  if (!action) {
  ...
  return { action: visibleAction, config }
}
...
async function setVisibleCustomActionConfig(store: TestStore) {
...
describe("selection toolbar requests", () => {
  ...
  it("renders the custom action tooltip as non-interactive and closes it on hover leave", async () => {
    ...
    const store = createStore()
    const { action } = await setVisibleCustomActionConfig(store)
    setSelectionState(store, { text: "Selected text" })
    renderWithProviders(<SelectionToolbarCustomActionButtons />, store)

    const trigger = screen.getByRole("button", { name: action.name })
    await openTooltip(trigger)

    const tooltip = document.querySelector("[data-slot='tooltip-content']")
    expect(tooltip).toHaveTextContent(action.name)
    expect(tooltip).toHaveClass("pointer-events-none")
