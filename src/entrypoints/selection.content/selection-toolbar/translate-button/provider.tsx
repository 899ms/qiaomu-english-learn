...
import { recordLearningTextEncounter } from "@/utils/word-learning/storage"
...
export function SelectionTranslationProvider({
  ...
  const wordLearningConfig = useAtomValue(configFieldsAtomMap.wordLearning)
  ...
  const runTranslation = useCallback(async (runId: number) => {
    ...
    if (wordLearningConfig.enabled) {
      void recordLearningTextEncounter(preparedText, "selection", {
        minimumLevel: wordLearningConfig.minimumLevel,
        hideKnownWords: wordLearningConfig.hideKnownWords,
      })
    }
    ...
    try {
      ...
      if (isLLMProviderConfig(providerConfig)) {
        ...
        const nextSnapshot = await translateWithLlm({
          ...
          onChunk: (data) => {
            if (runIdRef.current === runId) {
              setTranslatedText(normalizeEscapedNewlines(data.output))
        ...
        nextTranslatedText = normalizeEscapedNewlines(nextSnapshot.output)
        if (runIdRef.current === runId) {
        ...
        setThinking(null)
        nextTranslatedText = normalizeEscapedNewlines(await translateWithStandardProvider({
          text: preparedText,
          providerConfig,
          translateRequest,
        }))
  ...
  }, [resetTranslationState, selectionText, sourceSurface, translateRequest, wordLearningConfig])
        ...
        >
          ...
          <SelectionPopover.Body>
            <TranslationContent
              ...
              minimumWordLearningLevel={wordLearningConfig.minimumLevel}
              thinking={thinking}
            />
